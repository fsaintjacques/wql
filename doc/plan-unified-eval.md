...
/// Evaluate `program` against `input`.
///
/// - **Filter-only programs**: `output` is unused; pass `&mut []`.
///   Returns `EvalResult { output_len: 0, matched }`.
/// - **Project-only programs**: `matched` is always `true`.
///   Returns `EvalResult { output_len: n, matched: true }`.
/// - **Filter+project programs**: returns both fields populated.
///
/// # Buffer sizing
///
...
///
/// Returns:
///   0  — success; *out_len and *matched are populated.
///  -1  — error;   *errmsg (if non-null) is set.
///
/// For filter-only programs, pass output=NULL / output_len=0.
/// For project-only programs, *matched is always set to true (1).
///
...
Return code is `int` (0 = ok, -1 = error). Output goes through two out
parameters: `*out_len` for bytes written, `*matched` for predicate result.
`errmsg` follows existing convention (caller frees with `wql_errmsg_free`).

## Migration steps

1. **wql-runtime** — add `EvalResult` struct and `eval()` function.
   Rewrite `filter`, `project`, `project_and_filter` as thin wrappers around
   `eval` (keeps them compiling during transition, tests stay green).

2. **wql-capi** — add `wql_eval()`. Mark old execution functions
   `#[deprecated]`. Update the C header.
...
- `EvalResult` fields are always populated (no `Option`). The function inspects
...
- The C API returns an `int` error code rather than encoding multiple meanings
  into a single `int64_t` return value (the current `project_and_filter` uses
  -1 = filtered, -2 = error). This is cleaner and avoids sentinel values.
